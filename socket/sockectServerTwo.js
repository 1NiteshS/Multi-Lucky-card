import Game from '../models/gameModelTwo.js';
            ...
            try {
                if (timer.remainingTime > 0) {
                    ...
                    if (timer.remainingTime === mainTime - 2) {
                        const result = await getCurrentGameTwo();
                        if (result.success) {
                            gameID = result.data.gameId;
                            if (gameID === previousGameID) {
                                console.log('GameID unchanged, retrieving data again...');
                                const refreshedResult = await getCurrentGameTwo();
...
const startSocketTwo = (httpServer) => {
